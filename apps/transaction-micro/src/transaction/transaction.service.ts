...
export class TransactionService {
  ...
  async create(transactionInput: CreateTransactionInput): Promise<Transaction> {
    ...
    this.logger.log(
      `Transaction entity created with id: ${transactionEntity.transactionExternalId}`,
    );
  ...
  async findById(id: string): Promise<Transaction> {
    const transactionEntity = await this.transactionRepository.findOneBy({
  ...
  emitTransaction(transactionExternalId: string, value: number) {
    ...
    this.logger.log(
      `Create Transaction event has been emitted for id: ${transactionExternalId}`,
    );
  }
}
